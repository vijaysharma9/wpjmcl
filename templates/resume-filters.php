...
<form class="resume_filters">
	...
	<div class="search_resumes">
		...
		<div class="search_keywords resume-filter">
			...
			<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php _e( 'All Resumes', 'wp-job-manager-company-listings' ); ?>" value="<?php echo esc_attr( $keywords ); ?>" />
		</div>
		...
		<?php elseif ( $show_categories && get_option( 'resume_manager_enable_categories' ) && ! is_tax( 'resume_category' ) && get_terms( 'resume_category' ) ) : ?>
			<div class="search_categories resume-filter">
				...
				<?php else : ?>
					<?php wp_dropdown_categories( array( 'taxonomy' => 'resume_category', 'hierarchical' => 1, 'show_option_all' => __( 'Any category', 'wp-job-manager-company-listings' ), 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category ) ); ?>
				<?php endif; ?>
			</div>
		<?php endif; ?>
