...
<form class="company_filters">
	...
	<div class="search_companies">
		...
		<div class="search_keywords company-filter">
			...
			<input type="text" name="search_keywords" id="search_keywords" placeholder="<?php _e( 'All Companies', 'wp-job-manager-company-listings' ); ?>" value="<?php echo esc_attr( $keywords ); ?>" />
		</div>

		<div class="search_location company-filter">
			<label for="search_location"><?php _e( 'Location', 'wp-job-manager-company-listings' ); ?></label>
			<input type="text" name="search_location" id="search_location" placeholder="<?php _e( 'Any Location', 'wp-job-manager-company-listings' ); ?>" value="<?php echo esc_attr( $location ); ?>" />
		</div>
		...
		<?php elseif ( $show_categories && get_option( 'company_manager_enable_categories' ) && ! is_tax( 'company_category' ) && get_terms( 'company_category' ) ) : ?>
			<div class="search_categories company-filter">
				...
				<?php else : ?>
					<?php wp_dropdown_categories( array( 'taxonomy' => 'company_category', 'hierarchical' => 1, 'show_option_all' => __( 'Any category', 'wp-job-manager-company-listings' ), 'name' => 'search_categories', 'orderby' => 'name', 'selected' => $selected_category ) ); ?>
				<?php endif; ?>
			</div>
		<?php endif; ?>
