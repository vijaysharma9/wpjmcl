...
switch ( $company->post_status ) :
	case 'publish' :
		if ( company_listings_user_can_view_company( $company->ID ) ) {
	...
	break;
	default :
		do_action( 'company_listings_company_submitted_content_' . str_replace( '-', '_', sanitize_title( $company->post_status ) ), $company );
	break;
endswitch;
